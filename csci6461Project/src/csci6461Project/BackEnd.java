...
public class BackEnd extends Thread {
	...
	// converts shorts to strings, I had issues with the leading zeroes whenever checkbox 1 was clicked, 
	// I would get 32 numbers in the textfield
	// additionally, The length of PC / mar would increase at the first 4 opboxes so I had to put if statments
	// shorten them by taking away the first couple of digits depending on the length. 
	public String short_to_string(short num, int digits) { 
	...
	// still had issues with the size in pc and mar so I cut out the first 4 characters when the length
	// == 16. 
	public void updateGUI() {
		if (short_to_string(pc, 12).length() == 16) {
			gui.textField_pc.setText(short_to_string(pc, 12).substring(4));
		}
		else {
			gui.textField_pc.setText(short_to_string(pc, 12));
		}
		if (short_to_string(mar, 12).length() == 16) {
			gui.textField_mar.setText(short_to_string(mar, 12).substring(4));
		}
		else {
			gui.textField_mar.setText(short_to_string(mar, 12));
		}
		gui.textField_gpr0.setText(short_to_string(gpr0, 16));
		gui.textField_gpr1.setText(short_to_string(gpr1, 16));
		gui.textField_gpr2.setText(short_to_string(gpr2, 16));
		gui.textField_gpr3.setText(short_to_string(gpr3, 16));
		gui.textField_ixr1.setText(short_to_string(ixr1, 16));
		gui.textField_ixr2.setText(short_to_string(ixr2, 16));
		gui.textField_ixr3.setText(short_to_string(ixr3, 16));
		gui.textField_mbr.setText(short_to_string(mbr, 16));
		gui.textField_ir.setText(short_to_string(ir, 16));
		gui.textField_mfr.setText(short_to_string(mfr, 4));
		gui.textField_cc.setText(short_to_string(cc, 4));
	}
